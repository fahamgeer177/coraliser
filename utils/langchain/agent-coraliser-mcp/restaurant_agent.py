...
import urllib.parse  # Added for MCP server URL construction
...
from livekit.agents import mcp  # Import MCP integration
...
# MCP server connection details (added as per your request)
...
class Greeter(BaseAgent):
    def __init__(self, menu: str) -> None:
        ...
        super().__init__(
            instructions=(
                "You are the central interface agent that connects users with specialized agents to fulfill their queries.\n\n"
                "Your workflow:\n"
...
async def entrypoint(ctx: JobContext):
    ...
    session = AgentSession[UserData](
        ...
        vad=silero.VAD.load(),
        max_tool_steps=5,
        # Add MCP server connection here
        mcp_servers=[
