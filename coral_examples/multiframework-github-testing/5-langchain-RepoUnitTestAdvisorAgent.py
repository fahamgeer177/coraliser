...
async def create_repo_unit_test_advisor_agent(client, tools):
    prompt = ChatPromptTemplate.from_messages([
        ...
        3. Parse the message to extract the `repo` name, `owner`, `branch`, and the **list of target files** to evaluate, call `send_message(senderId=..., mentions=[senderId], threadId=..)` if any information was lack.
