...
async def create_codediff_review_agent(client, tools):
    prompt = ChatPromptTemplate.from_messages([
        ...
        **Loop**:
        ...
        3. Send the result via Call `send_message(senderId: 'codediff_review_agent', mentions: ['user_interaction_agent'])`.

        4. If the mention format is invalid or parsing fails, continue the loop silently.

        5. Do not create threads. Track `threadId` from mentions. Tools: {get_tools_description(tools)}"""),
